        ...
        match &message.command {
            &Command::PRIVMSG(ref chan, ref msg) => {
                if !chan.is_channel_name() || self.ui.has_chat_buf(chan)? {
                    self.ui.add_event_to_chat_buf(
                        chan,
                        Event::message(message.source_nickname(), chan, msg),
                    )?
                }
            }
            &Command::NOTICE(ref chan, ref msg) => {
                if !chan.is_channel_name() || self.ui.has_chat_buf(chan)? {
                    self.ui.add_event_to_chat_buf(
                        chan,
                        Event::notice(message.source_nickname(), chan, msg),
                    )?
                }
            }
            &Command::JOIN(ref chan, _, _) => {
                if !chan.is_channel_name() || self.ui.has_chat_buf(chan)? {
                    self.ui.add_event_to_chat_buf(
                        chan,
                        Event::joined(message.source_nickname(), chan),
                    )?
                }
            }
                ...
                if let Some(chan) = args.iter().find(|s| s.is_channel_name()) {
                    if let Some((suffix, _args)) = args.split_last() {
                        ...
                        for user in users {
                            ...
                            self.ui
                                .add_event_to_chat_buf(chan, Event::joined(Some(nickname), chan))?
                        }
                    }
                }
            }
